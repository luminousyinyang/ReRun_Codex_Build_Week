...
4. **Assets:** the P0 fixture references bundled, original scene art in `public/assets/scenes/`; no media-generation request is needed during a demo. A scene may provide `visualAsset`; when it does not, the renderer chooses a local scene-type fallback. Future asset jobs may cache stable character references and scene art by content hash, but they must never block playback.
