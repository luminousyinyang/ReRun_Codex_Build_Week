...
Ship a runnable, hosted Education-category web app by July 21, 2026, with a complete no-key judge experience and a truthful explanation of how Codex and GPT-5.6 were used. The P0 path is the only promise: **Photosynthesis demo -> CH 03 interactive episode -> active recall loop**.

## Scope

| Priority | Ship | Deliberately defer |
| --- | --- | --- |
| P0 | CRT shell and remote; bundled Photosynthesis course; recap; CH 03 narrative/beat/consequence flow; rewind; commercial review; ratings display; cliffhanger; paste-note live mode; responsive/a11y basics; judge docs | None |
...
2. Freeze `EpisodeSpec` and `ConceptGraph`; add fixtures and validation tests before UI work.
3. Implement the deterministic player and demo course, including every P0 branch.
...
| Jul 16 | Consequences, variant re-ask, rewind state, commercial review |
...
- The intended incorrect-answer, rewind, variant-answer, commercial, and cliffhanger loop works with keyboard and pointer input.
