...
describe("demo narration catalog", () => {
  it("derives every spoken demo and rewind line from the finalized fixtures", () => {
    const tracks = collectDemoNarrationTracks();
    const expectedKeys = new Set(
      demoShows.flatMap((show) => show.episode.scenes.flatMap((scene) => [scene.line, scene.simpler, scene.simplerAgain]
        .filter((line): line is string => Boolean(line))
        .map((line) => `${show.theme.voice}:${normalizeNarrationLine(line)}`))),
    );

    expect(tracks.map((track) => track.key)).toEqual([...expectedKeys].sort());
